...
def test_deactivate_member_excludes_from_default_list(admin_client, db):
    ...
    listed_removed = admin_client.get("/members", params={"active": "false"}, headers=headers, cookies=cookies).json()
    assert listed_removed["total"] == 1
    assert listed_removed["items"][0]["identity_email"] == "lawyer@acme.com"
