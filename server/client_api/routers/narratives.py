...
# case can generate/export, not office_manager-gated even though it produces
# a fee figure.
...
def export_narrative_pdf(
    ...
    """Renders the narrative to PDF and files it as a real Document on the
    case, internal folder by default (CLAUDE.md). This is what makes a
    narrative reachable by the client at all — sharing it afterwards is just
    the existing Documents reclassify-to-client-folder action, no separate
    visibility mechanism needed here.
    """
    case = get_assigned_case(case_id, tenant, membership, db)
    narrative = _get_case_narrative(case_id, narrative_id, tenant, db)

    pdf_bytes = build_narrative_pdf(case, narrative)
    filename = f"narrative-{narrative.id}.pdf"
