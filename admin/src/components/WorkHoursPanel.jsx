...
import { deleteWorkLog, listWorkLogs, updateWorkLog } from '../api/work_logs'
import { formatDate } from '../utils/format'
import './WorkHoursPanel.css'

// office_manager oversight view: every work log on the case, no assignment
// needed (same automatic access as Documents), with the same broad
// edit/delete authority as any assigned lawyer. No creation here — manual
// entry is the lawyer/client portal's job, this is oversight.
export default function WorkHoursPanel({ caseId, caseClosed }) {
  ...
  const [editDraft, setEditDraft] = useState({ date: '', hours: '', description: '' })
  ...
  return (
    <div className="card detail-card">
      <div className="detail-assign-header">
        <div className="detail-card-title">שעות עבודה בתיק</div>
      </div>
      {caseClosed && <div className="documents-closed-note">התיק סגור — לא ניתן לערוך או למחוק שעות.</div>}
